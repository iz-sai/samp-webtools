...
SampWT = (function(){
  ...
  function createDOM() {
    ...
    container.innerHTML =
      ...
      '<div id="sampwt-help"><a href="" target="_blank">Help</a></div>';
  ...
  var table = {
    ...
    underlineCoords: function() {
      ...
      this.node.select('col').each(function(col, index){
        if (col.hasClassName('coords')) indexes.push(index + 1); // +1 compensates added checkbox td
      });
